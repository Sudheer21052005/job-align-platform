...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login to proceed with authentication.
        ...
        # -> Click on 'Jobs' menu to expand and view available jobs.
        frame = context.pages[-1]
        # Click on Jobs menu to expand job options
        elem = frame.locator('xpath=html/body/div/div/div/div/div/nav/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on 'Jobs' menu to expand and view job options.
        frame = context.pages[-1]
        # Click on Jobs menu to expand job options
        elem = frame.locator('xpath=html/body/div/div/div/div/div/nav/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        # -> Click Logout to log out from job seeker account and then log in as recruiter.
        frame = context.pages[-1]
        ...
        # -> Input recruiter email and password, then click login button to authenticate.
        frame = context.pages[-1]
        # Input recruiter email for login
        elem = frame.locator('xpath=html/body/div/div/div/main/main/div/div/div/form/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('spcs0404@gmail.com')
        

        frame = context.pages[-1]
        # Input recruiter password for login
        elem = frame.locator('xpath=html/body/div/div/div/main/main/div/div/div/form/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('spj040506')
        

        frame = context.pages[-1]
        # Click login button to submit recruiter credentials
        elem = frame.locator('xpath=html/body/div/div/div/main/main/div/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        # -> Click Logout button to log out from job seeker account again.
        frame = context.pages[-1]
        # Click Logout button to log out from job seeker account
        elem = frame.locator('xpath=html/body/div/div/div/div/div/nav/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=AI Matching Score: 100% Perfect Match').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The test plan execution failed because the recruiter could not view all applicants or the AI-generated detailed candidate-job matching scores and summaries as expected.")
