...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login to access user account for resume upload testing.
        ...
        # -> Input email and password, then click login button to access user account.
        frame = context.pages[-1]
        ...
        # -> Click on 'Update Profile' to access resume upload section.
        frame = context.pages[-1]
        # Click on Update Profile to go to profile update page for resume upload
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Unsupported resume format detected. Please upload a valid resume file.').first).to_be_visible(timeout=30000)
        except AssertionError:
            raise AssertionError("Test failed: Uploading an unsupported or corrupted resume file did not display the expected error message indicating invalid file format or handle the error gracefully.")
