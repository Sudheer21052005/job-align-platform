...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login to proceed with recruiter login.
        ...
        # -> Click on 'Jobs' menu to expand and find 'Post Job' option.
        frame = context.pages[-1]
        # Click on Jobs menu to expand options
        elem = frame.locator('xpath=html/body/div/div/div/div/div/nav/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on 'Jobs' menu to expand it and find 'Post Job' option.
        frame = context.pages[-1]
        # Click on Jobs menu to expand options
        elem = frame.locator('xpath=html/body/div/div/div/div/div/nav/div').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=AI job requirements extracted successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The job creation and AI-powered job description analysis did not complete successfully as per the test plan.")
