...
async def run_test():
    ...
    try:
        ...
        # -> Input the valid registered email and password into the respective fields.
        frame = context.pages[-1]
        # Input valid registered email into the email field.
        ...
        # -> Try to open developer tools or use browser API to directly check for JWT token in storage or intercept network requests for token.
        frame = context.pages[-1]
        # Click Logout to end session and verify if token is cleared or to reset state.
        elem = frame.locator('xpath=html/body/div/div/div/div/div/nav/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click the login button to submit the login form and then verify JWT token presence.
        frame = context.pages[-1]
        # Click the Login button to submit the login form.
        elem = frame.locator('xpath=html/body/div/div/div/main/main/div/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        # -> Input the valid password into the password field and click the login button again.
        frame = context.pages[-1]
        # Input valid password into the password field.
        elem = frame.locator('xpath=html/body/div/div/div/main/main/div/div/div/form/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('spj040506')
        ...
        frame = context.pages[-1]
        # Click the Login button to submit the login form after filling password.
        elem = frame.locator('xpath=html/body/div/div/div/main/main/div/div/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
